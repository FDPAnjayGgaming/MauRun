<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="page-header-title">{{ __('Edit Event') }}</h2>
        </div>
    </x-slot>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('manage-events.update', $event->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_event" class="form-input-modern" value="{{ old('nama_event', $event->nama_event) }}" required>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                        <div>
                            <label class="form-label">Tanggal Pelaksanaan</label>
                            <input type="date" name="tanggal" class="form-input-modern" value="{{ old('tanggal', $event->tanggal) }}" required>
                        </div>
                        <div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" rows="4" class="form-input-modern" required>{{ old('deskripsi', $event->deskripsi) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($event->banner)
                            <div class="mb-3 relative inline-block">
                                <img src="{{ asset('storage/'.$event->banner) }}" alt="Banner Lama" class="w-40 rounded-xl border border-slate-200 shadow-sm">
                                <span class="absolute -top-2 -right-2 badge badge-info text-[10px]">Saat ini</span>
                            </div>
                        @endif
                        <div class="mt-1 flex justify-center rounded-xl border-2 border-dashed border-slate-300 px-6 py-6 hover:border-brand-400 transition-colors"
                            ...
                            <div class="text-center">
                                <svg x-show="!fileName" class="mx-auto h-8 w-8 text-slate-300" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor">
                                ...
                                <svg x-show="fileName" x-cloak class="mx-auto h-8 w-8 text-brand-500" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <div class="mt-2 text-sm text-slate-600">
                                    <label class="cursor-pointer text-brand-600 font-semibold hover:text-brand-500">
                                        <span x-show="!fileName">Ganti gambar</span>
                                        <span x-show="fileName" x-cloak>Ganti lagi</span>
                                ...
                                <p x-show="!fileName" class="text-xs text-slate-400 mt-1">Biarkan kosong jika tidak ingin mengubah.</p>
                    ...
                    <div class="form-group">
                        <label class="inline-flex items-center cursor-pointer gap-2">
                            <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-brand-600 shadow-sm focus:border-brand-300 focus:ring focus:ring-brand-200 focus:ring-opacity-50" {{ old('is_active', $event->is_active) ? 'checked' : '' }}>
                            <span class="text-sm text-slate-700">Aktifkan (Publish)</span>
                        </label>
                    </div>
